...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("Number of different words found in file:  " + wordList.size());
			System.out.println();
			if (wordList.size() == 0) {
			...
			wordList = alphabetSort(wordList);
	...
	/**
	 * Sorts a list of strings into lexicographical order, using selection sort
	 * and treating the list much like an array. In this program, the list only
	 * contains words made up of lower case letters, so lexicographic order is
	 * the same as alphabetical order.
	 */
	 ...
	 */
	private static ArrayList<String> alphabetSort(ArrayList<String> List) {
		int[] positions = new int[List.size()];
		int wordPos = -1;
		for (String word : List) {
			int newPos = 0;
			wordPos++;
			for (String wordComp : List) {
				int state = word.compareTo(wordComp);
				if (state > 1) {
					newPos++;
				}
			}
			positions[wordPos] = newPos;
		}
